...
uses gw.web.contacts.merge.MergeContactsAddressHolder
...
class MergeContactsWebUtil {
  ...
  /**
   * Creates a shallow copy of the {@link ABContact} and its Tags and PrimaryAddress.
  ...
  function createCopy(contact : ABContact) : ABContact {
    var newContact = contact.copyNonPersisting(contact)
    if (contact.PrimaryAddress != null) {
      newContact.PrimaryAddress = contact.copyNonPersisting(contact.PrimaryAddress)
    }
    for (tag in contact.Tags) {
      var newTag = contact.copyNonPersisting(tag)
   ...
   */
  function updateIsFromMerge(contact : ABContact) : boolean {
  ...
  private function mergeRetiredAddresses(contact : ABContact, retiredContact : ABContact,
    ...
    if (addressData.Included and not addressData.Primary) {
      contact.addAddress(primaryAddress)
    }
  }
