...
class DataSetsHelper extends DataSetsHelperBase
{
  ...
  private function _generateABContactsAndAddressesAndMessages() {
    ...
    //TrainingApp
	//adding all persons with lastname starting with A as employees of albertsons for Code generation and debugging student exercise
    addEmployeesForAlbertsons()
  }

  private function addEmployeesForAlbertsons() {
    gw.transaction.Transaction.runWithNewBundle(\ bundle -> {
      var employeesQuery = Query.make(ABPerson)
      var employees = employeesQuery.startsWith(ABPerson#LastName, "A", true).select()

      var albertsons = Query.make(ABCompany).compare(ABCompany#Name, Equals, "Albertson's").select().AtMostOneRow

      if(albertsons != null) {
        for(oneEmployee in employees) {
          bundle.add(oneEmployee)
          oneEmployee.Employer = albertsons
        }
      }

      albertsons = bundle.add(albertsons)
      albertsons.PrimaryContact = employees.firstWhere(\ person -> person.FirstName == "William" and person.LastName == "Andy")
  ...
  private function generateABPersonsAndMessages() {
    ...
    // TrainingApp
	createAndInitializeWilliamAndy(abAddress4)
  ...
  // TrainingApp
  private function createAndInitializeWilliamAndy(address : Address) {

    //Adding 2 secondary addresses for the Code generation and debugging instructor demo
    var santaMonicaBusiness = caBusinessAddressBuilder("1 Main Street", "Santa Monica", "90401")
    var marinaDelReyBilling = caBillingAddressBuilder("1 Old Harbor Lane", "Marina Del Rey", "90292")

    //Create and initialize William Andy
    var wAndy = new ABPersonBuilder().withFirstName("William").withLastName("Andy").withPrimaryAddress(address).withWorkPhone("209-666-2483").withCellPhone("772-492-6598")
        .withPrimaryPhoneType(PrimaryPhoneType.TC_WORK).withTaxID("123-45-6793").withTags(new ContactTagType[]{ContactTagType.TC_CLIENT})
        .withAddress(santaMonicaBusiness).withAddress(marinaDelReyBilling).create()

    //Adding 5 contact notes for the Introduction to Gosu instructor demo
    // Creating and initializing first contact note
    var note1 = new ContactNote()
    note1.Subject = "William Andy has an inquiry"
    note1.ContactNoteType = ContactNoteType.TC_GENERAL
    note1.Body = "William Andy has more questions related to data privacy"

    // Creating and initializing second contact note
    var note2 = new ContactNote()
    note2.Subject = "William Andy reported an issue"
    note2.ContactNoteType = ContactNoteType.TC_PROBLEM
    note2.Body = "William Andy has discovered a serious data entry issue"

    // Creating and initializing third contact note
    var note3 = new ContactNote()
    note3.Subject = "William Andy has a new licence"
    note3.ContactNoteType = ContactNoteType.TC_LICENSE
    note3.Body = "William Andy sent in his new licence information"

    // Creating and initializing fourth contact note
    var note4 = new ContactNote()
    note4.Subject = "New phone number"
    note4.ContactNoteType = ContactNoteType.TC_DATA_UPDATE
    note4.Body = "William Andy has a new phone number"

    // Creating and initializing fitfh contact note
    var note5 = new ContactNote()
    note5.Subject = "William Andy has questions"
    note5.ContactNoteType = ContactNoteType.TC_GENERAL
    note5.Body = "William Andy has many general questions"

    // Adding new notes to the contact notes array for the Introduction to Gosu instructor demo
    wAndy.addToContactNotes(note1)
    wAndy.addToContactNotes(note2)
    wAndy.addToContactNotes(note3)
    wAndy.addToContactNotes(note4)
    wAndy.addToContactNotes(note5)

    // Creating two new bank accounts for the Introduction to Gosu student exercise
    wAndy.addToBankAccounts(createBankAccount("ACME Credit Union", "123-987", "345678900", BankAccountType.TC_CHECKING))
    wAndy.addToBankAccounts(createBankAccount("National Bank", "745-222", "232323566", BankAccountType.TC_CHECKING))
  }

  function createBankAccount(bankName : String, routingNumber : String,
                             accountNumber : String, accountType : BankAccountType) : BankAccount {
  ...
  private function generateABPolicyPersons() {
	...
	// TrainingApp
	createAndInitializeEricAndy(abAddress96, "650-666-8023", PrimaryPhoneType.TC_WORK, "123-84-7704", "772-492-6598")
  ...
  private function createAndInitializeEricAndy(address:Address,
                                        workPhone:String,
                                        primaryPhoneType:PrimaryPhoneType,
                                        taxId:String,
                                        cellPhone:String) {
    gw.transaction.Transaction.runWithNewBundle( \b -> {
      var eAndy = new ABPolicyPersonBuilder()
          .withFirstName("Eric")
          .withLastName("Andy")
          .withPrimaryAddress(address)
          .withWorkPhone(workPhone)
      ...
      eAndy.addToBankAccounts(createBankAccount("ACME Credit Union", "123-999", "112233445", BankAccountType.TC_CHECKING))
      eAndy.addToBankAccounts(createBankAccount("ACME Credit Union", "225-999", "554433221", BankAccountType.TC_SAVINGS))
    })
  }
