...
(SELECT * FROM stats$sql_plan p, stats$sql_plan_usage u
   WHERE s.old_hash_value = u.old_hash_value and u.plan_hash_value = p.plan_hash_value AND p.operation = '${planPattern}' AND p.options = '${planOptionsPattern}' AND s.snap_id = p.snap_id)
UNION ALL
SELECT old_hash_value, ${columnToSum} AS ${columnToSum}
FROM stats$sql_summary s
